      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-9">
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentshow === 'CPUs' ? CPUs.map((p) => {if((p.socket === socket.msocket || socket.msocket=='all') && ( (p.supportedMemoryTypes.includes(ramtype as any)||ramtype=='all') || (items.Motherboard==undefined && items.RAM==undefined ) ))
            {/*console.log(p.supportedMemoryTypes, ramtype)*/;return (<BuilderProductCard key={p.id} product={p as any} />  )}}) :

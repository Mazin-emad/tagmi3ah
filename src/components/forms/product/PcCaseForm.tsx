...
export default function PcCaseForm({ categoryId }: PcCaseFormProps) {
  ...
  const onSubmit = (data: PcCaseFormData) => {
    ...
    createPcCase(
      ...
      {
        onSuccess: () => {
          toast.success("PC Case created successfully!");
          form.reset();
          setImageFile(null);
        },
        onError: (error: unknown) => {
          ...
          if (apiError?.fieldErrors) {
            Object.entries(apiError.fieldErrors).forEach(([field, message]) => {
              form.setError(field as keyof PcCaseFormData, { message });
            });
          }
          toast.error(apiError?.message || "Failed to create PC Case");
        },
      }
    );
  };
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Add New PC Case</CardTitle>
        <CardDescription>Create a new PC case product</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button type="submit" disabled={isPending} className="w-full">
              {isPending ? "Creating..." : "Create PC Case"}
            </Button>
          </form>
        </Form>
      </CardContent>
